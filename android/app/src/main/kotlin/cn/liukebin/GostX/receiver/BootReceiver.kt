...
class BootReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (prefs.getBoolean("lantern_proxy_running", false)) {
            LanternProxyService.start(context)
            return
        }
        ...
        if (wasRunning) {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    val repo = ConfigRepository(prefs)
                    if (repo.getActiveProfileId() == AnycastConfigBuilder.PROFILE_ID) {
                        runCatching {
                            AnycastAutoManager(context).refreshActiveProfile(repo)
                        }
                    }
